...
function addUser(message) {
  const node = el(`<div class="flex justify-end mb-2">
    <div class="bubble rounded-2xl px-4 py-2 bg-white text-black">${escapeHtml(message)}</div>
  </div>`);
  chatBox.appendChild(node); chatBox.scrollTop = chatBox.scrollHeight;
}
function addBotPending() {
  const node = el(`<div class="flex items-start gap-2 mb-2">
    <div class="size-8 rounded-2xl bg-white/10 flex items-center justify-center">🜏</div>
    <div class="bubble rounded-2xl px-4 py-2 glass border border-white/10 text-white/80"><span class="dot">…</span></div>
  </div>`);
  chatBox.appendChild(node); chatBox.scrollTop = chatBox.scrollHeight; return node.querySelector('.dot');
}
function addBot(message) {
  const node = el(`<div class="flex items-start gap-2 mb-2">
    <div class="size-8 rounded-2xl bg-white/10 flex items-center justify-center">🜏</div>
    <div class="bubble rounded-2xl px-4 py-2 glass border border-white/10">${renderMarkdown(message)}</div>
  </div>`);
  chatBox.appendChild(node); chatBox.scrollTop = chatBox.scrollHeight;
}
function escapeHtml(s){return s.replace(/[&<>\"']/g,m=>({"&":"&amp;","<":"&lt;",">":"&gt;","\"":"&quot;","'":"&#39;"}[m]));}
function renderMarkdown(s){return s
  .replace(/```([\s\S]*?)```/g,(m,code)=>`<pre class="overflow-x-auto p-3 rounded-xl bg-black/70 border border-white/10">${escapeHtml(code)}</pre>`)
  .replace(/\*\*(.+?)\*\*/g,'<strong>$1</strong>')
  .replace(/\*(.+?)\*/g,'<em>$1</em>')
  .replace(/`([^`]+)`/g,'<code class="px-1 rounded bg-black/60 border border-white/10">$1</code>')
  .replace(/\n/g,'<br>');}

async function send(message) {
  addUser(message);
  const dot = addBotPending();
  sendBtn.disabled = true; msg.value = '';
  try {
